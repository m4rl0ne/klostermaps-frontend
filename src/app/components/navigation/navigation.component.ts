...
export class NavigationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.mapService.getMap().subscribe(res => {
      ...
      let image = L.imageOverlay(unsafeImageUrl, bounds).addTo(this.map);
      ...
      L.marker(sol, { icon: defaultIcon }).addTo(this.map);
      this.map.setView([500,500], -1)

      var sol = L.latLng([ 250, 310 ]);
      L.marker(sol, { icon: defaultIcon }).addTo(this.map);
      this.map.setView([500,500], -1)
      ...
      var polyline = L.polyline([L.latLng([500,500]), L.latLng([250,310])], {color: 'red'}).addTo(this.map);
